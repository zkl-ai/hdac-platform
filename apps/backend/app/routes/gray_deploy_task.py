...
def create_deploy_task():
    ...
    task = GrayDeployTask(
        ...
        device_subset=body.get('deviceSubset'), # Save device subset (comma-separated string)
        status='pending',
    )
    db.session.add(task)
    db.session.flush()
...
def inference(id):
    ...
    # Optional: simulate which device handled it (randomly pick from subset if defined)
    device_id = None
    if t.device_subset:
        subset = [d.strip() for d in t.device_subset.split(',') if d.strip()]
        if subset:
            fake_device_id = random.choice(subset)
        else:
            fake_device_id = f"device-{random.randint(1, 5)}"
    else:
        fake_device_id = f"device-{random.randint(1, 5)}"
    ...
    if is_candidate:
        ...
        if v and v.avg_latency_ms:
            ...
            # Ensure a realistic minimum for simulation if DB value is too low (e.g. < 1ms placeholder)
            if latency < 5.0: 
                latency = 20.0
    ...
    # Record System Metric (Throttle: once per 5s per device)
    # We should filter by device_id to throttle per device
    last_sys = GraySystemMetric.query.filter_by(task_id=id, device_id=fake_device_id).order_by(GraySystemMetric.created_at.desc()).first()
    ...
    if not last_sys or (datetime.utcnow() - last_sys.created_at).total_seconds() > 5:
        try:
            ...
            db.session.add(GraySystemMetric(task_id=id, device_id=fake_device_id, cpu_percent=cpu, memory_percent=mem, gpu_percent=gpu))
        except Exception:
            pass
...
def get_metrics(id):
    ...
    # Build Query
    q_rec = GrayInferenceRecord.query.filter(
        GrayInferenceRecord.task_id == id,
        GrayInferenceRecord.created_at >= since
    )
    if device_id:
        q_rec = q_rec.filter(GrayInferenceRecord.device_id == device_id)
        
    records = q_rec.order_by(GrayInferenceRecord.created_at).all()
    ...
    # System metrics
    q_sys = GraySystemMetric.query.filter(
        GraySystemMetric.task_id == id, 
        GraySystemMetric.created_at >= since
    )
    if device_id:
        q_sys = q_sys.filter(GraySystemMetric.device_id == device_id)
        
    sys_metrics = q_sys.order_by(GraySystemMetric.created_at).all()
    
    # Get current config
    stage = GrayDeployStage.query.filter_by(task_id=id).first()
    target_ratio = stage.gray_ratio if stage else 0
    
    # Get available devices (distinct device_ids in recent records)
    # Ideally we should query this from GrayInferenceRecord or GraySystemMetric
    # For efficiency, we can just query distinct device_id from GraySystemMetric in last hour
    recent_devices = []
    
    # Strategy 1: If task has device_subset defined, use it!
    t = GrayDeployTask.query.get(id)
    if t and t.device_subset:
        recent_devices = [d.strip() for d in t.device_subset.split(',') if d.strip()]
    else:
        # Fallback: discover from metrics
        try:
            # Use a slightly larger window for device discovery
            dev_since = now - timedelta(hours=1)
            distinct_devs = db.session.query(GraySystemMetric.device_id).filter(
                GraySystemMetric.task_id == id,
                GraySystemMetric.created_at >= dev_since
            ).distinct().all()
            recent_devices = [r[0] for r in distinct_devs if r[0]]
        except Exception:
            pass
